...
const Chatbot = () => {
  ...
  const [messages, setMessages] = useState([
    {
      id: 1,
      text:
        "Hi! I'm your SkillSync assistant. I can guide you through Upload → Analyze → Builder → Download, help you find Jobs & Internships, and prepare for interviews.\n\nTry asking:\n• How do I analyze my resume?\n• Where can I see my ATS score?\n• How do Jobs work?\n• How do I use Resume Builder?",
      isBot: true,
      timestamp: new Date(),
    },
  ]);
  ...
  const generateBotResponse = (input) => {
    if (input.includes("hello") || input.includes("hi")) {
      return {
        id: messages.length + 2,
        text: `Hello ${user?.name || "there"}! How can I help with your resume today?`,
        isBot: true,
        timestamp: new Date(),
      };
    }
    ...
    if (input.includes("upload")) {
      return {
        id: messages.length + 2,
        text:
          "To upload a resume, open the Upload Resume page from the sidebar and select your PDF or DOCX file. SkillSync will analyze it automatically.",
        isBot: true,
        timestamp: new Date(),
      };
    }
    ...
    if (input.includes("ats") || input.includes("score")) {
      return {
        id: messages.length + 2,
        text:
          "ATS score shows how well your resume matches job requirements. Scores above 70% usually perform well in applicant tracking systems.",
        isBot: true,
        timestamp: new Date(),
      };
    }

    if (input.includes("job") || input.includes("intern")) {
      return {
        id: messages.length + 2,
        text:
          "Go to the Jobs & Internships page in the sidebar. Select your role and location to see opportunities powered by the JSearch API.",
        isBot: true,
        timestamp: new Date(),
      };
    }
    ...
    if (input.includes("interview")) {
      return {
        id: messages.length + 2,
        text:
          "SkillSync can generate interview questions based on your resume skills and experience to help you practice for real interviews.",
        isBot: true,
        timestamp: new Date(),
      };
    }
    ...
    if (input.includes("resume")) {
      return {
        id: messages.length + 2,
        text: `You currently have ${userResumes.length} resume${
          userResumes.length !== 1 ? "s" : ""
    ...
    return {
      id: messages.length + 2,
      text:
        "I can help you with resume analysis, ATS scores, interview preparation, and job search. What would you like to know?",
      isBot: true,
      timestamp: new Date(),
    };
  };
  ...
  return (
    <>
      {!isOpen && (
        <button
          onClick={() => setIsOpen(true)}
          className="fixed bottom-6 right-6 bg-primary-600 text-white p-4 rounded-full shadow-lg hover:bg-primary-700 transition"
        >
          <FiMessageCircle size={24} />
        </button>
      )}
      ...
      {isOpen && (
        <div
          className={`fixed bottom-6 right-6 w-96 bg-white rounded-xl shadow-2xl transition ${
            isMinimized ? "h-14" : "h-[500px]"
          }`}
        >
          {/* Header */}
